...
text-decoration: none; margin: auto; text-align: center; font-weight: 700; font-size: 18px">
        ...
        <div class="modal" :class="{mostrar:modal}">
            <div class="border" style="width: 800px; margin: auto; height: auto; background-color: #fff;overflow: hidden; border-radius: 20px; margin-top: 150px;padding: 0 0 20px 0">
                <div style="height: 15%; display: flex;align-items: center;justify-content: center" class="bg-indigo-800">
                    <span style="text-align: center; font-size: 25px; color: #fff; font-weight: 700">{{tituloModal}}</span>
                </div>
                <div style="height: 95%">
                    <div style="padding: 5px 50px;" >
                        <label for="" style="display: block; color: #1E40AF; font-weight: 600">Nombre:</label>
                        <input v-model="challenge.nombre" type="text" name="nombre" placeholder="Ejemplo: ¿Cómo podrias contribuir a la 4ta revolución industrial?" style="width: 100%; padding: 8px 8px" class="focus:outline-none rounded border">
                        <div v-if="errors && errors.nombre">
                            <small style="color: red;">{{errors.nombre[0]}}</small>
                        </div>
                    </div>
                    <div style="padding: 5px 50px;">
                        <label for="" style="display: block; color: #1E40AF; font-weight: 600">Descripción:</label>
                        <input v-model="challenge.descripcion" type="text" name="descripcion" placeholder="Ejemplo: Describe las estrategias que aplicarías" style="width: 100%; padding: 8px 8px" class="focus:outline-none rounded border">
                        <div v-if="errors && errors.descripcion">
                            <small style="color: red;">{{errors.descripcion[0]}}</small>
                        </div>
                    </div>
                    <div style="padding: 5px 50px;" :class="modificar ? 'ver' : ''">
                        <label for="" style="display: block; color: #1E40AF; font-weight: 600">Empresa:</label>
                        <select  v-model="challenge.empresa" name="empresa" style="width: 100%; padding: 8px 8px" class="focus:outline-none rounded border">
                            <option v-for="company in companies" v-bind:value="company.id">{{company.nombre}}</option>
                        </select>
                        <div v-if="errors && errors.empresa">
                            <small style="color: red;">{{errors.empresa[0]}}</small>
                        </div>
                    </div>
                    <div style="display: flex; justify-content: space-between ;padding: 5px 50px">
                        <button @click="closeModal()" type="button" class="btn btn-light rounded border" style="" data-dismiss="modal">Cerrar</button>
                        <button @click="save()" type="button" class="bg-pink-600 rounded" style="padding: 8px 15px; color: #fff; font-weight: 600;text-decoration: none" data-dismiss="modal">Guardar</button>
                    </div>
                </div>
            ...
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="text-center mb-1" style="font-size: 20px;color: #1E40AF;font-weight: 700" id="exampleModalLabel">{{tituloModal}}</h5>
